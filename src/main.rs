...
fn s3run(matches: &ArgMatches) -> io::Result<()> {
    let mode = match matches.subcommand() {
        ...
        ("server", Some(m)) => {
            Mode::Server(value_t!(m, "server-port", u16).unwrap(),
                         m.value_of("client-address").unwrap().to_owned(),
                         value_t!(m, "client-port", u16).unwrap())
        }
        ("client", _) => Mode::Client,
        _ => {
            return Err(io::Error::new(io::ErrorKind::Other,
                                      "Choose mode to run: master, server, client"));
        }
    };
    ...
    let matches =
        App::new(env!("CARGO_PKG_NAME"))
            ...
            .arg(Arg::with_name("tunnel-file-name")
                     .help("Name of the files to use for transfer data: tunnel.in \
                            tunnel.out, this is just the name, not the extension.")
                     .long("tunnel-file-name")
                     .default_value("tunnel"))
            .arg(Arg::with_name("proxy")
                     .help("Proxy server")
                     .long("proxy"))
            .subcommand(SubCommand::with_name("master").about("Start the tunnel in interactive \
                                                               session to start multiple \
                                                               different servers"))
            .subcommand(SubCommand::with_name("server")
                            .about("Start the tunnel as one listener listening on one \
                                    specific port")
                ...
                val.parse::<u16>()
                    ...
                    .map_err(|_| format!("Cannot parse {} to u16", val))
                            ...
                            .arg(Arg::with_name("client-address")
                                     .long("client-address")
                                     .help("Address where to connect on a new \
                                            connection for the client mode.")
                                     .takes_value(true)
                                     .default_value("127.0.0.1"))
                            .arg(Arg::with_name("client-port")
                                     .long("client-port")
                                     .help("Port where to connect on a new connection \
                                            for the client mode.")
                                     .takes_value(true)
                                     .default_value("22")
                                     .validator(|val| {
            ...
            .subcommand(SubCommand::with_name("client").about("Start the tunnel in client mode"))
            .get_matches();
